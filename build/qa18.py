"""QA ролика 18: зоны карточек, пересечение графики с субтитрами, наложение строк."""
import os
import sys
...
from storyboard18 import OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS, shot_at
...
def line_overlaps():
    ...
    for i in range(len(CAPS) - 1):
        ...
        slot = render18.SLOTS[render18.slot_for(shot_at(CAPS[i][0])[2])]
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render18.graphics_layer(kind, t - ts)
        if gl is None:
            continue
