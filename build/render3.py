"""Сборка ролика 2 («актуарий»). Речь не резана — резы только по картинке.

Отличие от render1.py (который был упрощён по прямой просьбе на ролик 1):
здесь используется полный набор брендкита — сток + собственная графика на
сетке (числа-ревилы R5a/R5b, каскад и разряд-хайлайт R10).
"""
...
from style import *
from storyboard3 import SHOTS, CAPS, SLOTS, DUR, LABELS

BUILD = os.path.dirname(os.path.abspath(__file__))
SRC = "/Users/vladimirkalajcidi/reels_good2/videos/2/source.mov"
VIDEO_DIR = os.path.dirname(SRC)
A1 = f"{BUILD}/assets/aroll_3_A1.mp4"
...
def prep_stock():
    ...
    for i, (t0, t1, kind, prm) in enumerate(SHOTS):
        ...
        dst = f"{STOCK_PREP_DIR}/sb3_{i}_{prm['clip']}.mp4"
...
def main():
    ...
    tmp = f"{BUILD}/assets/_video3.mp4"
    ff = subprocess.Popen(
