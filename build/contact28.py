"""Контакт-лист ключевых кадров ролика 28 — смысловая проверка графики глазами."""
import os
import sys
...
from storyboard28 import SRC, SHOTS, shot_at
from style import FPS
import render28

OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "/private/tmp/claude-501/-Users-vladimirkalajcidi-reels-good/089ff210-4e91-4ce6-826e-a3d0cb3c6a87/scratchpad/contact28"
...
for t in TIMES:
    ...
    def read_stock():
        c = cv2.VideoCapture(f"{render28.STOCK_DIR}/stock_{prm['clip']}.mp4")
        c.set(cv2.CAP_PROP_POS_MSEC, (prm.get("ss", 0) + (t - t0)) * 1000)
        _, s = c.read()
        c.release()
        return s

    canvas = render28.compose(kind, prm, fr, t, t0, read_stock)
