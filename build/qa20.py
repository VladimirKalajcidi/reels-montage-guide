"""QA ролика 20: зоны карточек, пересечение графики с субтитрами, наложение строк,
...
from storyboard20 import (OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS,
                          shot_at)
from style import CARD_A, CARD_B, FPS
import render20

ALPHA = 40
WORDS_JSON = "/Users/vladimirkalajcidi/reels_good/videos/20/source.json"
...
def line_overlaps():
    ...
    for i in range(len(CAPS) - 1):
        ...
        slot = render20.SLOTS[render20.slot_for(shot_at(CAPS[i][0])[2])]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render20.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render20.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render20.graphics_layer(kind, t - ts)
        if gl is None:
            continue
