"""Звук ролика 32: SFX + тихая музыка + исходная речь без резки.

Постель: whoosh на каждой из 24 склеек, импакт под двумя ревилами чисел
(белое 100, синее 99), тики под каскадами фигур и маркеров ряда.
"""
import os
import subprocess
import sys
...
from storyboard32 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
...
def main():
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под ревилом числа (100 и 99)
        add(im, t, peak * 0.070)
    for t in CASCADES:                           # тики под каскадами фигур ряда
        for k in range(3):
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=1.60dB,alimiter=limit=0.80:level=disabled",
