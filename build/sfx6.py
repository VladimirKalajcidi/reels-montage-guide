"""Звук ролика 6: синтез SFX + музыкальная подложка + финальный микс.
Уровни — delivery-specs.md §5 (уже откорректированные, не поднимать).
"""
import os, sys, subprocess, wave
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from storyboard6 import SHOTS, SRC, OUT as FINAL_OUT

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video6.mp4"
OUT = FINAL_OUT
SR = 48000

# точки ревила чисел (R5a/R5b) и точки каскадов (R10 со стаггером)
NUM_REVEALS = [9.261, 12.054, 15.549]
CASCADES = [27.881, 36.701]
...
def main():
    tmp = f"{BUILD}/assets/_voice6.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-2.0:LRA=7,"
                           "alimiter=limit=0.79:attack=5:release=50:level=false",
