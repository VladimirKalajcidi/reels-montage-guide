"""QA ролика 13: зоны карточек, пересечение графики с субтитрами, наложение строк."""
import os
import sys
...
from storyboard17 import OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS, shot_at
...
def line_overlaps():
    ...
    for i in range(len(CAPS) - 1):
        ...
        slot = render17.SLOTS[render17.slot_for(shot_at(CAPS[i][0])[2])]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render17.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render17.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render17.graphics_layer(kind, t - ts)
        if gl is None:
            continue
