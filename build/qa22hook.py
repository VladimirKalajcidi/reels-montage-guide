"""QA версий ролика 22 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa22hook.py h1
    python3 qa22hook.py h2
"""
import json
import os
import subprocess
...
def body_identical(key, n=14):
    ...
    версии (до деформации) с _video_dilemma.mp4 по PSNR."""
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_dilemma{key}.mp4")
...
def deformation(key):
    ...
    t_main = 47.0                                  # план с лицом внутри тела
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_dilemma{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    ...
    cfg = HK.HOOKS[key]
    hw = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/22/"
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/22/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
...
def face_metrics():
    return {"основной": (396, 671, 264), "hook1": (395, 678, 283), "hook2": (393, 655, 279)}


def music_unique():
    used = {"v1 (dilemma_edit)": "song3.mp3"}
