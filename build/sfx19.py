"""Звук ролика 19: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard19 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
...
def main():
    tmp = f"{BUILD}/assets/_voice_qr.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_qr.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_qr.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=0.6dB,alimiter=limit=0.80:level=disabled",
