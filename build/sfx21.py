"""Звук ролика 21: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard21 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_porog.mp4"
# song3 (110с) длиннее ролика (40.07с) — подложка идёт без петли, шва не существует
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song3.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_porog.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_porog.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_porog.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=-0.1dB,alimiter=limit=0.80:level=disabled",
