"""Звук ролика 25: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard25 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_morley.mp4"
# song2 (335с) длиннее ролика — подложка идёт без петли, шва не существует в принципе
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song2.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_morley.wav"
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под ревилом результата
        add(im, t, peak * 0.070)
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_morley.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_morley.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=0.2dB,alimiter=limit=0.80:level=disabled",
