"""QA ролика 23: зоны карточек, пересечение графики с субтитрами, наложение строк,
синхрон субтитров и резов с пословными таймингами.

Пословные тайминги лежат в `videos/23/words.json` — плоский список слов,
собранный по речевым отрезкам (см. шапку storyboard23).
"""
import json
import os
import sys
...
from storyboard23 import (OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS,
                          WORDS, shot_at)
...
def line_overlaps():
    ...
    Считается по тем же данным, что и рендер: шаг блока из render23.BLOCKS,
    ...
    for i in range(len(CAPS)):
        bid, pos, _, _, yoff = render23.BLOCKS[i]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render23.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render23.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render23.graphics_layer(kind, t - ts)
        if gl is None:
            continue
...
def blue_and_lime():
    ...
    return dict(blue_shots=["pickOne"], blue_numbers=0,
                lime="импульс поправки часов на плане fourthClock")


def geometry():
    """Построение обязано быть настоящим, а не «похожим»:

    1. P_TRUE лежит ровно на всех трёх сферах;
    2. каждая окружность целиком помещается в зону графики;
    3. третья сфера режет окружность пересечения ровно в двух точках,
       одна из которых — P_TRUE, вторая — Q, и они разнесены читаемо;
    4. Q лежит строго внутри первых двух сфер (у точки на окружности
       пересечения есть ненулевая z-составляющая).
    """
    import math
    from storyboard23 import SAT, RAD, P_TRUE, P_ALT, ell_point
    gx0, gx1 = GFX_X
    gy0, gy1 = GFX_ZONE
    bad = []
    for k in ("S1", "S2", "S3"):
        if abs(math.dist(SAT[k], P_TRUE) - RAD[k]) > 1e-6:
            bad.append(("P не на сфере", k))
        c, r = SAT[k], RAD[k]
        if not (c[0] - r >= gx0 and c[0] + r <= gx1
                and c[1] - r >= gy0 and c[1] + r <= gy1):
            bad.append(("окружность вне зоны", k))
    roots, prev = [], None
    for i in range(36001):
        g = i / 100.0
        f = math.dist(ell_point(g), SAT["S3"]) - RAD["S3"]
        if prev is not None and (prev < 0 <= f or prev >= 0 > f):
            roots.append(g)
        prev = f
    if len(roots) != 2:
        bad.append(("третья сфера режет окружность не в 2 точках", len(roots)))
    hits = [ell_point(g) for g in roots]
    if not any(math.dist(h, P_TRUE) < 1.0 for h in hits):
        bad.append(("P_TRUE не среди точек пересечения", None))
    sep = max(math.dist(h, P_TRUE) for h in hits) if hits else 0.0
    if sep < 150:
        bad.append(("кандидаты слишком близко", round(sep, 1)))
    for k in ("S1", "S2"):
        if math.dist(P_ALT, SAT[k]) >= RAD[k]:
            bad.append(("Q не внутри сферы", k))
    return len(bad), bad, dict(sep=round(sep, 1),
                               radii={k: round(v, 1) for k, v in RAD.items()})
