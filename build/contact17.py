...
from storyboard17 import SRC, SHOTS, shot_at
from style import FPS
import render17

OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "/tmp/contact17"
...
for t in TIMES:
    ...
    def read_stock():
        c = cv2.VideoCapture(f"{render17.STOCK_DIR}/stock_{prm['clip']}.mp4")
        c.set(cv2.CAP_PROP_POS_MSEC, (prm.get("ss", 0) + (t - t0)) * 1000)
        _, s = c.read()
        c.release()
        return s

    canvas = render17.background(kind, prm, fr, t, read_stock)
