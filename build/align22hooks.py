"""Пословные тайминги дублей хука ролика 22 — той же схемой, что и основной
...
VID = "/Users/vladimirkalajcidi/reels_good/videos/22"
BUILD = "/Users/vladimirkalajcidi/reels_good/build"

# Правые границы последних отрезков сняты по огибающей, а не по длительности
# файла: в hook1 речь смолкает на 7.08 при файле 7.195, в hook2 — на 3.20 при
# 3.292. Растягивать выдачу модели на хвост из дыхания значит уводить субтитры
# на 0.1с к концу фразы.
TAKES = {
    "hook1": [(0.302, 3.624), (4.119, 7.080)],
    "hook2": [(0.225, 3.200)],
}

model = whisper.load_model("large-v3")
for name, spans in TAKES.items():
    wav = f"{BUILD}/assets/_align_{name}.wav"
