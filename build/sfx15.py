"""Звук ролика 15: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard15 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_garfield.mp4"
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song2.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_garfield.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_garfield.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_garfield.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    # отдавал -14.9 LUFS. Догоняем volume + alimiter.
                    # level=disabled обязателен: у alimiter автонормализация включена по
                    # умолчанию, с ней та же цепочка давала -12.2 LUFS и пик +0.2 dBFS.
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           # 1.1 dB (как в ролике 14) дало -13.8 LUFS — на 0.2 громче
                           # цели; 0.9 dB сажает ровно в -14 (замерено на готовом mp4)
                           "volume=0.9dB,alimiter=limit=0.80:level=disabled",
