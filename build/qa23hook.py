"""QA версий ролика 23 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa23hook.py h1
    python3 qa23hook.py h2
"""
import json
import os
import subprocess
...
def body_identical(key, n=14):
    ...
    версии (до деформации) с _video_gps.mp4 по PSNR."""
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_gps{key}.mp4")
...
def deformation(key):
    ...
    t_main = 20.0                                  # план с лицом внутри тела (A2 19.12-22.11)
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_gps{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    ...
    cfg = HK.HOOKS[key]
    hw = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/23/"
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/23/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
...
def face_metrics():
    """Крупность дублей детектором (cx, линия глаз, ширина лица), после hflip.
    Замерено hook23.face_metrics(); кропом НЕ правится — числа называются в сдаче."""
    return {"основной": (390, 636, 291), "hook1": (395, 639, 291), "hook2": (394, 641, 292)}


def music_unique():
    used = {"v1 (gps_edit)": "song3.mp3"}
