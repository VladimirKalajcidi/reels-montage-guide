"""Звук ролика 17: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard17 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_luhn.mp4"
# song2 длиннее ролика — подложка идёт без петли, шва не существует в принципе
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song2.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_luhn.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_luhn.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_luhn.wav"
