"""Звук ролика 20: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard20 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_popcorn.mp4"
...
def main():
    tmp = f"{BUILD}/assets/_voice_popcorn.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_popcorn.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_popcorn.wav"
