"""Контакт-лист ключевых кадров ролика 24 — смысловая проверка графики глазами."""
import os
import sys
...
from storyboard24 import SRC, SHOTS, shot_at
from style import FPS
import render24

OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "/tmp/contact24"
...
for t in TIMES:
    ...
    def read_stock():
        c = cv2.VideoCapture(f"{render24.STOCK_DIR}/stock_{prm['clip']}.mp4")
        c.set(cv2.CAP_PROP_POS_MSEC, (prm.get("ss", 0) + (t - t0)) * 1000)
        _, s = c.read()
        c.release()
        return s

    canvas = render24.compose(kind, prm, fr, t, t0, read_stock)
