"""Звук ролика 22: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard22 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_dilemma.mp4"
# song3 (110с) длиннее ролика — подложка идёт без петли, шва не существует в принципе
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song3.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_dilemma.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_dilemma.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_dilemma.wav"
