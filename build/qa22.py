"""QA ролика 22: зоны карточек, пересечение графики с субтитрами, наложение строк,
синхрон субтитров и резов с пословными таймингами.

Пословные тайминги лежат в `videos/22/words.json` — плоский список слов,
собранный по речевым отрезкам (см. шапку storyboard22).
"""
import json
import os
import sys
...
from storyboard22 import (OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS,
                          WORDS, shot_at)
...
def line_overlaps():
    ...
    Считается по тем же данным, что и рендер: шаг блока из render22.BLOCKS,
    ...
    for i in range(len(CAPS)):
        bid, pos, _, _, yoff = render22.BLOCKS[i]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render22.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render22.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render22.graphics_layer(kind, t - ts)
        if gl is None:
            continue
...
def blue_and_lime():
    ...
    return dict(blue_shots=["eqResult", "verdict", "together", "couldHave"],
                blue_numbers=0, lime="связь между комнатами на плане deal")
