"""Звук ролика 36: SFX + тихая музыка + исходная речь без резки.

Постель: whoosh на каждой из 23 склеек, импакт под белым ревилом 1/2, под
крестом на отпавшем исходе и под двумя синими ответами (1/3 и 1/2), тики под
каскадами элементов квадрата исходов.
"""
import os
import subprocess
import sys
...
from storyboard36 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
...
def main():
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под ревилом и выводом
        add(im, t, peak * 0.070)
