"""Звук ролика 33: SFX + тихая музыка + исходная речь без резки.

Постель: whoosh на каждой из 27 склеек, импакт под четырьмя ревилами чисел
(белое 100, белое 50, дробь 1/2^100, синее 31%), тики под каскадами коробок.
"""
import os
import subprocess
import sys
...
from storyboard33 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
...
def main():
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под ревилом числа
        add(im, t, peak * 0.070)
    for t in CASCADES:                           # тики под каскадами коробок
        for k in range(3):
