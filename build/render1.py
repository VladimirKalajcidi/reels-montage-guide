"""Сборка ролика 1 («квонт»). Речь не резана — резы только по картинке.

Отличие от общего движка: минимум собственной графики (одна цифра-ревил),
вставки — сток-видео И фото (фото — по прямой просьбе на этот ролик).
"""
...
# источник 720x1280: кроп во всю ширину, высота под пропорцию карточки A (870x1380)
...
STOCK_CAP_CARD = (BX, BY, BW, 230)  # верхняя треть карточки B (R2: "в верхней трети карточки")
...
def prep_stock():
    ...
    for i, (t0, t1, kind, prm) in enumerate(SHOTS):
        if kind != "stock":
            continue
        src = f"{STOCK_DIR}/{prm['clip']}.mp4"
        dst = f"{STOCK_PREP_DIR}/sb1_{i}_{prm['clip']}.mp4"
...
def prep_photos():
    """Фото-вставка → карточка B, кроп до 1.385:1, то же притемнение, что у видео."""
    out = {}
    for i, (t0, t1, kind, prm) in enumerate(SHOTS):
        if kind != "photo":
            continue
        src = f"{STOCK_DIR}/{prm['clip']}.jpg"
        img = Image.open(src).convert("RGB")
        img = fit_cover(img, BW, BH)
        arr = np.asarray(img).astype(np.float32) / 255.0
        # eq=brightness=-0.22:contrast=1.08:saturation=0.90 (то же, что у видео-вставок)
        arr = (arr - 0.5) * 1.08 + 0.5 - 0.22
        gray = arr.mean(axis=2, keepdims=True)
        arr = gray + (arr - gray) * 0.90
        arr = np.clip(arr, 0, 1)
        out[i] = Image.fromarray((arr * 255).astype(np.uint8), "RGB")
    return out
...
def label_for(t, kind):
    """R7 — служебная подпись под карточкой B (сток/фото со счётным смыслом)."""
...
def money_layer(t):
    """Единственная цифра-ревил ролика — плоская тёмная карточка A вместо сетки."""
    t0 = MONEY["t0"]
    lt = t - t0
    L, T = [], []
    card = Image.new("RGBA", (CW, CH), (8, 8, 10, 255))
    L.append(card)
    T += num_items(t, t0 + 0.05, MONEY["digits"], (CX + CW // 2, CY + CH // 2 - 40), 128, True)
    T.append(label_item(MONEY["sub"], (CX + CW // 2, CY + CH // 2 + 120), 64, "r", blue=True,
                        opacity=min(1.0, max(0.0, lt - 0.35) / 0.3)))
    return L, T
...
def main():
    ...
    tmp = f"{BUILD}/assets/_video1.mp4"
    ff = subprocess.Popen(
    ...
    for f in range(NF):
        ...
        if kind == "A1":
            ...
            base = photo_files[shot_idx]
            dur = t1 - t0
            p = min(1.0, max(0.0, lt / max(0.01, dur)))
            zoom = 1.0 + 0.045 * p  # едва заметный дрейф, чтобы фото не читалось стоп-кадром
            zw, zh = int(BW * zoom), int(BH * zoom)
            zimg = base.resize((zw, zh), Image.LANCZOS)
            ox, oy = (zw - BW) // 2, (zh - BH) // 2
            zimg = zimg.crop((ox, oy, ox + BW, oy + BH))
            canvas.paste(zimg, (BX, BY), maskB)
        elif kind == "money":
            layers, titems = money_layer(t)
            for L in layers:
