"""Звук ролика 27: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard27 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_lace.mp4"
...
def main():
    tmp = f"{BUILD}/assets/_voice_lace.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_lace.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_lace.wav"
