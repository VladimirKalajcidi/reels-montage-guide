"""Сборка ролика 5 («квантовые вычисления»). Речь не резана — резы только по картинке.

Скопировано с render38.py (ролик 3, «криптограф») без изменения движка — только
пути, импорт раскадровки и поддержка R6 (крупное слово в карточке, `topword`),
которой не было у render38.py и которая нужна storyboard39.
"""
...
from style import *
from storyboard39 import SHOTS, CAPS, SLOTS, DUR, LABELS

BUILD = os.path.dirname(os.path.abspath(__file__))
SRC = "/Users/vladimirkalajcidi/reels_good2/videos/5/source.mov"
VIDEO_DIR = os.path.dirname(SRC)
A1 = f"{BUILD}/assets/aroll_39_A1.mp4"
...
# источник 720x1280 (совпадает с роликом 3): кроп во всю ширину, высота под
...
def prep_stock():
    ...
    for i, (t0, t1, kind, prm) in enumerate(SHOTS):
        ...
        dst = f"{STOCK_PREP_DIR}/sb39_{i}_{prm['clip']}.mp4"
...
def main():
    ...
    tmp = f"{BUILD}/assets/_video39.mp4"
    ff = subprocess.Popen(
