"""Звук ролика 28: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard28 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_envelopes.mp4"
# song3 (110с) длиннее ролика — подложка идёт без петли, шва не существует
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song3.mp3"
SR = 48000


def main():
    tmp = f"{BUILD}/assets/_voice_envelopes.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_envelopes.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_envelopes.wav"
