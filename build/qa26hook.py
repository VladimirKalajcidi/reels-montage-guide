"""QA версий ролика 26 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa26hook.py h1
"""
import json
import os
import subprocess
...
def body_identical(key, n=14):
    ...
    версии (до деформации) с _video_tsp.mp4 по PSNR."""
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_tsp{key}.mp4")
...
def deformation(key):
    ...
    t_main = 49.5                                  # план с лицом внутри тела:
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_tsp{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    ...
    cfg = HK.HOOKS[key]
    hw = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/26/"
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/26/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
...
def face_metrics():
    ...
    return {"основной": (402, 643, 283), "hook1": (408, 650, 285),
            "hook2": (412, 650, 286)}


def music_unique():
    used = {"v1 (tsp_edit)": "song2.mp3"}
