"""Звук ролика 11: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard11 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
...
def main():
    tmp = f"{BUILD}/assets/_voice_fermat.wav"
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под число-ревилом
        add(im, t, peak * 0.070)
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_fermat.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_fermat.wav"
