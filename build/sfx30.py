"""Звук ролика 30: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard30 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
...
def main():
    ...
    for t in NUM_REVEALS:                        # импакт под ревилом числа
        add(im, t, peak * 0.070)
    for t in CASCADES:                           # тики под каскадами узлов графа
        for k in range(3):
