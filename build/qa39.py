"""Автопроверки ролика 5 («квантовые вычисления») — delivery-specs.md §6. Обе должны дать ноль."""
import os, sys
import numpy as np
import cv2
...
from storyboard39 import SHOTS, DUR
import render39 as R

OUT = "/Users/vladimirkalajcidi/reels_good2/videos/5/quantum_edit.mp4"
...
# В этом ролике нет составной R10-графики (только простой R5b-ревил на сетке, который не
# пересекается с субтитрами — под ним подписи нет вообще, см. caption_items: kind=="num" -> []).
# Единственный элемент переднего плана поверх лица — крупное слово R6 (topword), оно рисуется
# в фиксированной полосе y~440 (см. brand-kit.md: "стена над головой"), а обычные субтитры на
# лице сидят на базовой линии y=1305 и блоками уходят вверх максимум на несколько строк.
# Проверяем пиксельно: полоса topword и фактические боксы субтитров не должны пересекаться.
...
if __name__ == "__main__":
    print("=== QA ролик 5 («квантовые вычисления») ===")
