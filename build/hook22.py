"""Ролик 22 («дилемма заключённых»), альтернативные хуки.

Тело ролика (всё с кадра 180) не пересобирается — берётся готовыми кадрами из
assets/_video_dilemma.mp4 и склеивается за новым хуком. Заново рисуется только хук.

    python3 hook22.py h1
    python3 hook22.py h2

Точка реза: кадр 180 (6.000с) — граница планов `A2` -> `stock`, то есть конец
старого начала «иногда действовать логично для себя — это самый нелогичный
выбор для двоих людей сразу. и вот очень известный пример». Планы 1-3 старого
хука и их субтитры выброшены целиком.

`cut_f = ceil(5.98 * 30)` = 180, а не `round` = 179: граница плана стоит на
дробном кадре 179.4, и кадр 179 — это ещё последний кадр ВЫБРАСЫВАЕМОГО плана
с лицом. Проверка машинная, в qa22hook.old_hook_gone.

Передача эстафеты. Тело начинается новым предложением с новым подлежащим:
«двух подозреваемых допрашивают в разных комнатах». Ни одного местоимения
и ни одной отсылки к словам старого хука в первых фразах тела нет —
подставлять слова в дубли не пришлось, обе версии режутся по одной точке.

hook1 кончается на «это математическая и социальная задачка как раз на это» —
та же функция, что у старого «и вот очень известный пример»: обещание примера.
hook2 кончается на «называется дилеммой заключённого» — название задачи,
после которого «двух подозреваемых допрашивают» читается как её условие.
...
Крупность дублей (детектор, после hflip, медиана по всему дублю):
    основной  cx 396  линия глаз 671  ширина лица 264
    hook1     cx 395  линия глаз 678  ширина лица 283  (+7px, +7.2%)
    hook2     cx 393  линия глаз 655  ширина лица 279  (-16px, +5.7%)
Кропом это НЕ правится: оба дубля идут через те же FRAMINGS, что и основной
исходник, числа названы в сдаче.

Графика хука. В старом начале стоял план `selfboth` (две фигуры, под каждой
галка «его выбор логичен», общий итог перечёркнут) — это визуальная интрига
ролика, она сохранена в обоих новых хуках и ложится на их слова точно так же:
hook1 «хотите как лучше, а получается как обычно», hook2 — название задачи.
...
from style import *
import render22 as R
import storyboard22 as SB
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VIDEO_DIR = "/Users/vladimirkalajcidi/reels_good/videos/22"
BODY = f"{BUILD}/assets/_video_dilemma.mp4"
AUDIO_DIR = "/Users/vladimirkalajcidi/reels_good/audios"
SRC = SB.SRC
SR = 48000

CUT_T = 5.98                                   # граница планов A2 -> stock
CUT_F = math.ceil(CUT_T * FPS - 1e-6)          # 180 — первый кадр тела
CUT = CUT_F / FPS                              # 6.000с
TARGET_LUFS = -14.2                            # замер сданного dilemma_edit.mp4
...
HOOKS = {
    "h1": dict(
        ...
        # дубль 215 кадров (7.167с), речь 0.30-7.08 по огибающей: хвост дубля
        # пустой, обрезать нечего. Хвост 0.087с + 0.360с в голове тела
        # = пауза 0.447с перед первым словом тела.
        src_ss=0.0,
        frames=215,
        out=f"{VIDEO_DIR}/dilemma_hook1.mp4",
        ...
        post_a=None,
        title="«у вас тоже такое бывает, что вы хотите как лучше, а получается "
              "как обычно? так вот, это математическая и социальная задачка "
              "как раз на это»",
        face_cx=395, face_eye=678, face_d=283,
        # структура старого начала сохранена: лицо -> графика -> лицо.
        shots=[
            (0 / 30,   42 / 30,  "A1", {}),
            (42 / 30,  110 / 30, "selfboth", {}),
            (110 / 30, 157 / 30, "A2", {}),
            (157 / 30, 215 / 30, "A1", {}),
        ],
        caps=[
            (0.30, 0.88, [("у вас тоже", R_, 48)], "A"),
            (0.88, 1.23, [("такое бывает", R_, 52)], "A"),

            (1.43, 1.94, [("что вы хотите", R_, 48)], "G"),
            (1.94, 2.54, [("как ", R_, 46), ("лучше", S_, 62)], "G"),
            (2.56, 3.00, [("а получается", R_, 48)], "G"),
            (3.00, 3.62, [("как ", R_, 46), ("обычно", S_, 62)], "G"),

            (4.12, 5.23, [("так вот это математическая", R_, 40)], "A"),

            # старт по кадру реза, а не по 5.23: слово «и» начинается ровно
            # на границе планов, и фраза иначе на один кадр повисает в конце
            # предыдущего плана и тащит его блок за собой
            (157 / 30, 6.40, [("и социальная ", R_, 42), ("задачка", S_, 54)], "A"),
            (6.40, 7.08, [("как раз на это", R_, 46)], "A"),
        ],
    ),
    "h2": dict(
        version=3,
        src=f"{VIDEO_DIR}/hook2.mov",
        # дубль 99 кадров (3.300с), речь 0.23-3.20. Хвост 0.100с + 0.360с
        # в голове тела = пауза 0.460с перед первым словом тела.
        src_ss=0.0,
        frames=99,
        out=f"{VIDEO_DIR}/dilemma_hook2.mp4",
        ...
        post_a="atempo=1.02",
        title="«эта математическая задачка называется дилеммой заключённого»",
        face_cx=393, face_eye=655, face_d=279,
        shots=[
            (0 / 30,  47 / 30, "A1", {}),
            (47 / 30, 99 / 30, "selfboth", {}),
        ],
        caps=[
            (0.23, 1.58, [("эта математическая задачка", R_, 44)], "A"),
            # 3 слова одной фразой: разбить нечем — «заключённого» осталось бы
            # одиночным словом, а это нарушение brand-kit §5 (2-4 слова).
            (1.58, 3.20, [("называется ", R_, 44), ("дилеммой заключённого", S_, 48)], "G"),
        ],
    ),
}
...
def build_video(key, cfg, tmp):
    """Кадры хука рисуем, кадры тела берём готовыми из _video_dilemma.mp4.
...
def wav_of(path, start=0.0):
    ...
    tmp = f"{BUILD}/assets/_tmp_hook22.wav"
...
def build_audio(key, cfg, nf_hook, vid, out):
    ...
    if m > 0.99:
        mix *= 0.99 / m
    mix_path = f"{BUILD}/assets/_mix_dilemma{key}.wav"
...
def apply_post_v(key, cfg):
    ...
    src = f"{BUILD}/assets/_video_dilemma{key}.mp4"
    vf = cfg.get("post_v")
    if not vf:
        return src
    dst = f"{BUILD}/assets/_video_dilemma{key}_post.mp4"
...
def main(key):
    cfg = HOOKS[key]
    tmp = f"{BUILD}/assets/_video_dilemma{key}.mp4"
    ...
    csp = f"{BUILD}/test/hook22_{key}_sheet.jpg"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "remux":
        mux(apply_post_v(k, HOOKS[k]), f"{BUILD}/assets/_mix_dilemma{k}.wav",
