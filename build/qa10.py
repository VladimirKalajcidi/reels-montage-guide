"""QA ролика 10: зоны карточек и пересечение графики с субтитрами."""
import os
import sys
...
from storyboard10 import OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, shot_at
...
def line_overlaps():
    ...
    for i in range(len(CAPS) - 1):
        if CAPS[i + 1][0] - CAPS[i][1] <= 0.30:
            ...
            slot = render10.SLOTS[render10.slot_for(shot_at(CAPS[i][0])[2])]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render10.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render10.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render10.graphics_layer(kind, t - ts)
        if gl is None:
            continue
