"""Звук ролика 7: голос из source.mp4 + тихие whoosh/impact/tick, музыка, loudnorm.

Голосовая дорожка не режется — берётся целиком, к ней подмешивается «постель».
Уровни SFX — проверенные значения из delivery-specs.md §5.
"""
import os
import subprocess
import sys
...
from storyboard7 import SRC, OUT, SHOTS, NUM_REVEALS, CASCADES
...
def low_whoosh(dur=0.42):
    ...
    a = np.exp(-2 * np.pi * 180 / SR)          # ФНЧ 180 Гц
    y = np.zeros(n)
    prev = 0.0
    for i in range(n):
    ...
    # саб не громче 0.45 от шумовой части — иначе бьёт по ушам
...
def impact(dur=0.50):
    n = int(SR * dur)
    t = np.linspace(0, dur, n, endpoint=False)
    ph = 2 * np.pi * np.cumsum(92 * np.exp(-t * 11) + 41) / SR
...
def main():
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_secretary.wav"
        ...
        xf = int(0.25 * SR)                       # петля с кроссфейдом на шве
        ...
        track *= (vr / mr) * (10 ** (-19 / 20))   # −19 dB от RMS голоса, без дакинга
