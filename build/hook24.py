"""Ролик 24 («регрессия к среднему»), альтернативный хук.

Тело ролика (всё с кадра 144) не пересобирается — берётся готовыми кадрами из
assets/_video_regress.mp4 и склеивается за новым хуком. Заново рисуется только хук.

    python3 hook24.py h1

Точка реза: кадр 144 (4.800с) — граница планов `stock` -> `A2`, то есть конец
старого начала «у очень высоких родителей дети в среднем чуть ниже, а у очень
богатых в среднем не такие успешные». Планы 1-3 старого хука и их субтитры
выброшены целиком.

`cut_f = ceil(4.797 * 30)` = 144, а не `round` = 144 — здесь оба совпадают,
но правило то же: граница плана стоит на дробном кадре 143.91, и кадр 143 —
это ещё последний кадр ВЫБРАСЫВАЕМОГО плана со стоком. Проверка машинная,
в qa24hook.old_hook_gone.

Передача эстафеты. Тело начинается с «и дело тут не в слабых генах, а в чистой
статистике». Указательное «тут» отсылает к самому явлению, а новый хук это
явление проговаривает целиком («у очень высоких родителей в среднем дети чуть
ниже, а у очень богатых, например, в среднем не такие успешные») — антецедент
на месте, подставлять слова в дубль не пришлось.

Хук говорит то же самое первое предложение другими словами и добавляет заход
«я думаю, вы точно замечали, что…». Поэтому убирается ровно одно предложение
старого начала, и его конец совпадает с границей плана кадр в кадр.

Формат дубля совпадает с основным исходником по всем тегам: 720x1280, yuv420p,
color_range=tv, bt709/bt709/bt709, 30 fps. Приведение цветового пространства
не требуется — кадры читаются прямо из .mov и идут в штатный тракт A-roll
без промежуточного кодека.

Крупность дублей (детектор, после hflip, медиана по всему дублю):
    основной  cx 398  линия глаз 639  ширина лица 283
    hook1     cx 390  линия глаз 638  ширина лица 279  (-8px по центру, -1.4%)
Кропом это НЕ правится, да и править нечего: дубли сняты с одной точки,
расхождение внутри разброса самого детектора. Числа названы в сдаче.

Графика хука. В старом начале стояли план `tallPair` (линия среднего, точка
родителя высоко, точка ребёнка ниже) и стоковая гора купюр — это визуальная
интрига ролика, оба сохранены в новом хуке и ложатся на его слова точно так же.
...
VIDEO_DIR = "/Users/vladimirkalajcidi/reels_good/videos/24"
BODY = f"{BUILD}/assets/_video_regress.mp4"
AUDIO_DIR = "/Users/vladimirkalajcidi/reels_good/audios"
SRC = SB.SRC
SR = 48000

CUT_T = 4.797                                  # граница планов stock -> A2
CUT_F = math.ceil(CUT_T * FPS - 1e-6)          # 144 — первый кадр тела
CUT = CUT_F / FPS                              # 4.800с
TARGET_LUFS = -14.1                            # замер сданного regress_edit.mp4
...
HOOKS = {
    "h1": dict(
        ...
        # дубль 209 кадров (6.967с), речь 0.060-6.960 по огибающей: хвост дубля
        # пустой, обрезать нечего.
        src_ss=0.0,
        frames=209,
        out=f"{VIDEO_DIR}/regress_hook1.mp4",
        ...
        post_a=None,
        title="«я думаю, вы точно замечали, что у очень высоких родителей "
              "в среднем дети чуть ниже, а у очень богатых, например, "
              "в среднем не такие успешные»",
        face_cx=390, face_eye=638, face_d=279,
        # структура старого начала сохранена: лицо -> сетка -> сток.
        # хвост добит графическим планом, а не стоп-кадром лица.
        shots=[
            (0 / 30,   44 / 30,  "A1", {}),
            (44 / 30,  125 / 30, "tallPair", {}),
            (125 / 30, 209 / 30, "stock", {"clip": "47005", "ss": 5.0}),
        ],
        caps=[
            (0.000, 0.422, [("я думаю", R_, 50)], "A"),
            (0.422, 1.448, [("вы точно ", R_, 48), ("замечали", S_, 56)], "A"),

            # старт по кадру реза, а не по 1.448: слово «у» начинается за 19мс
            # до границы планов, и фраза иначе на кадр повисает в конце
            # предыдущего плана и уезжает в слот лица
            (44 / 30, 2.815, [("у очень высоких родителей", R_, 44)], "G"),
            (2.815, 3.318, [("в среднем", R_, 48)], "G"),
            (3.318, 4.162, [("дети чуть ", R_, 46), ("ниже", S_, 60)], "G"),

            (4.323, 5.208, [("у очень ", R_, 46), ("богатых", S_, 58)], "B"),
            (5.570, 6.133, [("в среднем не", R_, 46)], "B"),
            (6.133, 6.957, [("такие успешные", R_, 50)], "B"),
        ],
    ),
}
...
def build_video(key, cfg, tmp):
    """Кадры хука рисуем, кадры тела берём готовыми из _video_regress.mp4.
...
def wav_of(path, start=0.0):
    ...
    tmp = f"{BUILD}/assets/_tmp_hook24.wav"
...
def build_audio(key, cfg, nf_hook, vid, out):
    ...
    if m > 0.99:
        mix *= 0.99 / m
    mix_path = f"{BUILD}/assets/_mix_regress{key}.wav"
...
def apply_post_v(key, cfg):
    ...
    src = f"{BUILD}/assets/_video_regress{key}.mp4"
    vf = cfg.get("post_v")
    if not vf:
        return src
    dst = f"{BUILD}/assets/_video_regress{key}_post.mp4"
...
def main(key):
    cfg = HOOKS[key]
    tmp = f"{BUILD}/assets/_video_regress{key}.mp4"
    ...
    csp = f"{BUILD}/test/hook24_{key}_sheet.jpg"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "remux":
        mux(apply_post_v(k, HOOKS[k]), f"{BUILD}/assets/_mix_regress{k}.wav",
