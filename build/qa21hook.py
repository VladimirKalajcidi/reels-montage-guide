"""QA версий ролика 21 с другими хуками.

Часть проверок идёт по раскадровке хука (до рендера), часть — по готовому файлу.

    python3 qa21hook.py h1
    python3 qa21hook.py h2
"""
import json
import os
import subprocess
...
from hook21 import HOOKS, CUT, CUT_F, lts_for
...
def bind(cfg):
    """Подменить раскадровку в render21 на хуковую (как это делает hook21)."""
    R.CAPS = cfg["caps"]
...
def old_hook_gone():
    """От старого начала не должно остаться ни кадра: кадр реза обязан быть
    первым кадром СЛЕДУЮЩЕГО плана, а не последним кадром выброшенного."""
    prev = SB.shot_at((CUT_F - 1) / FPS)
    first = SB.shot_at(CUT_F / FPS)
    dropped = SB.SHOTS[:SB.SHOTS.index(first)]
    return dict(cut_f=CUT_F, cut=round(CUT, 4),
                shot_before=prev[2], shot_after=first[2],
                different=prev is not first,
                dropped=[k for _a, _b, k, _p in dropped],
                dropped_caps=sum(1 for c in SB.CAPS if c[0] < CUT))


def seam_pause(cfg):
    """Пауза между последним словом хука и первым словом тела.
    Норма по delivery-specs §6 — 0.10-0.20с, как между предложениями внутри тела."""
    words = hook_words(cfg)
    hook_end = max(w[1] for w in words)
    hook_len = cfg["frames"] / FPS
    body_first = 5.200                      # «это» — первое слово тела после реза
    return dict(hook_speech_end=round(hook_end, 3),
                hook_block=round(hook_len, 4),
                tail=round(hook_len - hook_end, 3),
                body_head=round(body_first - CUT, 4),
                pause=round((hook_len - hook_end) + (body_first - CUT), 3))


def body_identical(cfg):
    """Тело версии кадр в кадр совпадает с уже сданным роликом.

    Сравниваем немую сборку версии (до деформации) с _video_porog.mp4:
    у недеформированной части PSNR обязан быть «бесконечным» (кадры копировались
    без перерисовки), допускаем только шум кодека.
    """
    import os as _os
    mid = f"{BUILD}/assets/_video_porog{[k for k, v in HOOKS.items() if v is cfg][0]}.mp4"
    if not _os.path.exists(mid):
        return None
    a = cv2.VideoCapture(mid)
    b = cv2.VideoCapture(f"{BUILD}/assets/_video_porog.mp4")
    ...
    a.release()
    b.release()
    return dict(frames_checked=checked, worst_psnr=round(float(worst), 1))
...
if __name__ == "__main__":
    ...
    print("старый хук выброшен:", old_hook_gone())
    print("пауза на стыке:", seam_pause(cfg))
    ...
    if os.path.exists(cfg["out"]):
        ...
        print("тело кадр в кадр:", body_identical(cfg))
