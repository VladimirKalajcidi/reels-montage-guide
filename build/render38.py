"""Сборка ролика 3 («криптограф»). Речь не резана — резы только по картинке.

Скопировано с render3.py (ролик 2, «актуарий») без изменения движка — только
пути и импорт раскадровки. Числа — только R5a/R5b (см. storyboard38.py).
"""
...
from style import *
from storyboard38 import SHOTS, CAPS, SLOTS, DUR, LABELS

BUILD = os.path.dirname(os.path.abspath(__file__))
SRC = "/Users/vladimirkalajcidi/reels_good2/videos/3/source.mov"
VIDEO_DIR = os.path.dirname(SRC)
A1 = f"{BUILD}/assets/aroll_38_A1.mp4"
OUT = "/Users/vladimirkalajcidi/reels_good2/videos/3/crypto_edit.mp4"
...
def prep_stock():
    ...
    for i, (t0, t1, kind, prm) in enumerate(SHOTS):
        ...
        dst = f"{STOCK_PREP_DIR}/sb38_{i}_{prm['clip']}.mp4"
...
GRAPHIC_KINDS = ("num", "cascade", "decimal")
...
# зона субтитров над графикой на сетке (brand-kit.md §5): y 300..600, левым флагом x167
...
def label_for(t, kind):
    """R7 — служебная подпись под карточкой B."""
    if kind != "stock":
        return []
...
def main():
    ...
    tmp = f"{BUILD}/assets/_video38.mp4"
    ff = subprocess.Popen(
