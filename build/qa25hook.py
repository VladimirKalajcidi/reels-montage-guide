"""QA версий ролика 25 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa25hook.py h1
    python3 qa25hook.py h2
"""
import json
import os
import subprocess
...
def head_trim(key):
    """Срез с головы первого плана тела идёт только по тишине.

    Речь не режется: между границей плана и первым словом тела в дубле
    сплошная тишина, и `head_trim_f` обязан целиком лежать внутри неё.
    Заодно проверяем, что укороченный план не стал короче 1.0с.
    """
    cfg = HK.HOOKS[key]
    n = cfg.get("head_trim_f", 0)
    cut = HK.cut_of(cfg)
    words = json.load(open(SB.WORDS))
    hit = [w["word"] for w in words if w["start"] < cut and w["end"] > HK.CUT]
    shot = SB.shot_at(cut)
    return dict(trim_frames=n, trim_sec=round(n / FPS, 3),
                cut_f=HK.cut_f_of(cfg), cut=round(cut, 4),
                speech_in_trim=hit,
                first_body_shot=shot[2],
...
def body_identical(key, n=14):
    ...
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_morley{key}.mp4")
...
def deformation(key):
    ...
    t_main = 31.8                                  # план с лицом внутри тела (A1 31.14-32.95)
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_morley{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    ...
    cfg = HK.HOOKS[key]
    hw = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/25/"
                        f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
    body_words = json.load(open(SB.WORDS))
    cut = HK.cut_of(cfg)
    first = next(w for w in body_words if w["start"] >= cut)
    dt = cfg["frames"] / FPS - cut
    speed = 1.02 if "setpts" in (cfg.get("post_v") or "") else 1.0
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/25/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
...
def face_metrics():
    ...
    return {"основной": (392, 642, 286), "hook1": (382, 652, 293), "hook2": (402, 648, 291)}


def music_unique():
    used = {"v1 (morley_edit)": "song2.mp3"}
