"""QA версий ролика 28 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa28hook.py h1
    python3 qa28hook.py h2
"""
import json
import os
import subprocess
...
def _pcm(path):
    ...
    raw = sp.run(["ffmpeg", "-v", "error", "-i", path, "-ac", "1", "-ar", "48000",
                  "-f", "s16le", "-"], capture_output=True).stdout
    return np.frombuffer(raw, np.int16).astype(np.float32) / 32768


def _amp(a, t, win=0.010, sr=48000):
    i = max(0, int(t * sr))
    return float(np.abs(a[i:i + int(win * sr)]).max())


def _onset_after(a, t, thr=0.02, step=0.002, limit=1.0):
    """Первый момент после t, где огибающая поднимается выше порога."""
    t2 = t
    while t2 < t + limit:
        if _amp(a, t2) > thr:
            return round(t2, 4)
...
def _offset_before(a, t, thr=0.02, step=0.002, limit=1.5):
    """Последний момент до t, где огибающая была выше порога."""
    t2 = t
    while t2 > t - limit and t2 > 0:
        if _amp(a, t2 - 0.010) > thr:
            return round(t2, 4)
        t2 -= step
    return None
...
def head_trim(key):
    """Срез с головы первого плана тела — по огибающей, а не по whisper.

    На границе плана 5.418 whisper ставит начало слова «перед», но по звуку
    там ещё звучит «так» из выбрасываемого «итак»: огибающая в 5.418 = 0.43.
    Поэтому проверяем по звуку: (1) рез стоит в провале между словами,
    (2) до реза тишина держится не меньше 25мс, (3) первый звук тела приходит
    после реза, то есть ни одно слово тела не срезано, (4) укороченный план
    не стал короче 1.0с.
    """
    cfg = HK.HOOKS[key]
    n = cfg.get("head_trim_f", 0)
    cut = HK.cut_of(cfg)
    a = _pcm(SB.SRC)
    onset = _onset_after(a, cut)
    prev_end = _offset_before(a, cut)
    words = json.load(open(SB.WORDS))
    whisper_first = next(w for w in words if w["start"] >= HK.CUT_T - 0.01)
    shot = SB.shot_at(cut)
    return dict(trim_frames=n, cut_f=HK.cut_f_of(cfg), cut=round(cut, 4),
                amp_at_cut=round(_amp(a, cut), 4),
                amp_at_shot_boundary=round(_amp(a, HK.CUT_T), 4),
                prev_word_ends=prev_end,
                silence_before_cut=round(cut - prev_end, 4) if prev_end else None,
                body_first_sound=onset,
                body_sound_after_cut=(onset is not None and onset >= cut),
                cut_in_silence=_amp(a, cut) < 0.02,
                whisper_says_first_word=(whisper_first["word"],
                                         whisper_first["start"]),
                first_shot=shot[2],
                first_shot_len=round(shot[1] - cut, 2))


def body_identical(key, n=14):
    """Тело кадр в кадр совпадает со сданной версией: сверяем немой монтаж
    версии (до деформации) с _video_morley.mp4 по PSNR."""
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_envelopes{key}.mp4")
    body = cv2.VideoCapture(BODY)
    total = int(body.get(cv2.CAP_PROP_FRAME_COUNT)) - HK.cut_f_of(cfg)
    worst = (1e9, None)
    for i in np.linspace(0, total - 2, n).astype(int):
        ver.set(cv2.CAP_PROP_POS_FRAMES, cfg["frames"] + int(i))
        body.set(cv2.CAP_PROP_POS_FRAMES, HK.cut_f_of(cfg) + int(i))
        oka, a = ver.read()
        okb, b = body.read()
        if not (oka and okb):
            continue
...
def deformation(key):
    ...
    dt = cfg["frames"] / FPS - HK.cut_of(cfg)
    speed = 1.02 if "setpts" in post else 1.0
    t_main = 34.8                                  # план с лицом внутри тела (A1 33.84-35.90)
    ...
    raw = cfg["frames"] + (int(video_facts(BODY)["frames"]) - HK.cut_f_of(cfg))
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_envelopes{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    """Пауза между последним звуком хука и первым звуком тела — по огибающей.

    Считать по словам whisper тут нельзя: на стыке он и промахивается
    (см. head_trim), и не знает про хвост тишины в дубле хука.
    """
    cfg = HK.HOOKS[key]
    cut = HK.cut_of(cfg)
    speed = 1.02 if "setpts" in (cfg.get("post_v") or "") else 1.0
    hook_a = _pcm(cfg["src"])
    body_a = _pcm(SB.SRC)
    block = cfg["frames"] / FPS
    hook_end = _offset_before(hook_a, min(block, len(hook_a) / 48000 - 0.02))
    body_onset = _onset_after(body_a, cut)
    pause = (block - hook_end) + (body_onset - cut)
    hw = json.load(open("/Users/vladimirkalajcidi/reels_good/videos/28/"
                        f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
    bw = json.load(open(SB.WORDS))
    return dict(hook_last_word=hw[-1]["word"],
                hook_speech_ends=hook_end, hook_block_ends=round(block, 4),
                hook_tail=round(block - hook_end, 4),
                body_first_word=next(w["word"] for w in bw
                                     if w["end"] > cut),
                body_head_silence=round(body_onset - cut, 4),
                pause=round(pause / speed, 3),
                ok=0.10 <= pause / speed <= 0.20)
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/28/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
    said = [norm(w["word"]) for w in src_words]
    # объявленные подмены окончаний: whisper на безударном хвосте слова
    # ошибается, слово в кадре пишется грамматически верно (см. hook25.HOOKS)
    endings = {norm(k): norm(v) for k, v in (cfg.get("asr_endings") or {}).items()}
    bad, fixed = [], []
    i = 0
    for t0, _t1, runs, _slot in cfg["caps"]:
        ...
        for w in text.split():
            ...
            if j < len(said):
                i = j + 1
                continue
            alt = endings.get(norm(w))
            if alt:
                j = i
                while j < len(said) and said[j] != alt:
                    j += 1
                if j < len(said):
                    i = j + 1
                    fixed.append((t0, w, alt))
                    continue
            bad.append(("нет в речи", t0, w))
    return dict(violations=len(bad), sample=bad[:6], ending_overrides=fixed)
...
def face_metrics():
    ...
    return {"основной": (388, 690, 292), "hook1": (402, 703, 298), "hook2": (420, 700, 296)}


def music_unique():
    used = {"v1 (envelopes_edit)": "song3.mp3"}
...
if __name__ == "__main__":
    ...
    print("срез головы тела:    ", head_trim(key))
