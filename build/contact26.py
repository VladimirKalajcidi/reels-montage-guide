"""Контакт-лист ключевых кадров ролика 26 — смысловая проверка графики глазами."""
import os
import sys
...
from storyboard26 import SRC, SHOTS, shot_at
from style import FPS
import render26

OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "/private/tmp/claude-501/-Users-vladimirkalajcidi-reels-good/a20c8dc2-37a7-42d3-977b-18c7aa48a7be/scratchpad/contact26"
...
for t in TIMES:
    ...
    def read_stock():
        c = cv2.VideoCapture(f"{render26.STOCK_DIR}/stock_{prm['clip']}.mp4")
        c.set(cv2.CAP_PROP_POS_MSEC, (prm.get("ss", 0) + (t - t0)) * 1000)
        _, s = c.read()
        c.release()
        return s

    canvas = render26.compose(kind, prm, fr, t, t0, read_stock)
