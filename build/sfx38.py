"""Звук ролика 3 («криптограф»): синтез SFX + микс с голосом. Уровни — delivery-specs.md §5."""
import os, sys, subprocess, wave
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from storyboard38 import SHOTS, DUR, NUMBERS

BUILD = os.path.dirname(os.path.abspath(__file__))
SRC = "/Users/vladimirkalajcidi/reels_good2/videos/3/source.mov"
VID = f"{BUILD}/assets/_video38.mp4"
OUT = "/Users/vladimirkalajcidi/reels_good2/videos/3/crypto_edit.mp4"
SR = 48000

# impact — на ревиле синего числа (R5b)
NUM_REVEALS = [n["t0"] + 0.05 for n in NUMBERS if n["blue"]]
...
def main():
    tmp = f"{BUILD}/assets/_voice38.wav"
    ...
    music_path = "/Users/vladimirkalajcidi/reels_good2/audios/song3.mp3"
    if os.path.exists(music_path):
        mw = f"{BUILD}/assets/_music38.wav"
        ...
        print("музыка: трек не найден, подкладка пропущена —", music_path)

    mix = voice + bed
    m = np.abs(mix).max()
    if m > 0.99:
        mix *= 0.99 / m
    out = f"{BUILD}/assets/_mix38.wav"
