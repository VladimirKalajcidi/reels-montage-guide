"""QA версий ролика 27 с другими хуками (delivery-specs §6, блок «Версия с другим хуком»).

    python3 qa27hook.py h1
"""
import json
import os
import subprocess
...
def body_identical(key, n=14):
    ...
    версии (до деформации) с _video_lace.mp4 по PSNR."""
    cfg = HK.HOOKS[key]
    ver = cv2.VideoCapture(f"{BUILD}/assets/_video_lace{key}.mp4")
...
def deformation(key):
    ...
    t_main = 41.5                                  # план с лицом внутри тела:
...
def seam_click(key):
    ...
    cfg = HK.HOOKS[key]
    wav = f"{BUILD}/assets/_mix_lace{key}.wav"
    f = wave.open(wav)
    n = f.getnframes()
...
def hook_body_pause(key):
    ...
    Точка реза квантуется по кадру (CUT), а слова в whisper — по времени
    речи, поэтому слово тела может начаться чуть раньше CUT и всё ещё
    звучать в момент склейки. В этом случае паузы нет вовсе: берём первое
    слово, которое ещё звучит на CUT или начинается позже него."""
    cfg = HK.HOOKS[key]
    hw = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/27/"
                        f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
    body_words = json.load(open(SB.WORDS))
    first = next(w for w in body_words if w["end"] > HK.CUT)
    dt = cfg["frames"] / FPS - HK.CUT
    speed = 1.02 if "setpts" in (cfg.get("post_v") or "") else 1.0
    gap = max(0.0, first["start"] - HK.CUT)
    return dict(hook_last_word=hw[-1]["word"], hook_ends=hw[-1]["end"],
                body_first_word=first["word"],
                body_word_spans_cut=first["start"] <= HK.CUT,
                pause=round(gap / speed, 3))
...
def hook_captions(key):
    ...
    src_words = json.load(open(f"/Users/vladimirkalajcidi/reels_good/videos/27/"
                               f"words_{os.path.basename(cfg['src']).split('.')[0]}.json"))
...
def bright_outside(key):
    ...
    Граница хук->тело определяется по номеру кадра, а не по непрерывному
    времени: `fps`-фильтр ускорения пересэмплирует кадры по номеру источника
    (floor), и на стыке это на доли кадра расходится с оценкой по времени —
    ровно один кадр из полутора тысяч классифицировался неверно (проверено
    визуально: кадр целиком внутри CARD_A, ложное срабатывание было в самой
    проверке, не в ролике)."""
    cfg = HK.HOOKS[key]
    dt = cfg["frames"] / FPS - HK.CUT
    speed = 1.02 if "setpts" in (cfg.get("post_v") or "") else 1.0
    crop = "crop" in (cfg.get("post_v") or "")
    hook_frames_out = int(cfg["frames"] / speed) if speed != 1.0 else cfg["frames"]
...
def face_metrics():
    ...
    return {"основной": (347, 677, 313), "hook1": (347, 670, 347),
            "hook2": (343, 685, 329)}


def music_unique():
    used = {"v1 (lace_edit)": "song3.mp3"}
...
if __name__ == "__main__":
    ...
    print("уникализация треков: ", music_unique())
