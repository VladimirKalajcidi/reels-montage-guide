"""Звук ролика 35: SFX + тихая музыка + исходная речь без резки.

Постель: whoosh на каждой из 23 склеек, импакт под пятью синими выводами
(крест «R нарушает условие», галочка «R подходит», вынужденная копия внутри R,
замыкание петли, крест на коробке из произвольного свойства), тики под
каскадами элементов. Чисел в ролике не звучит, поэтому ревилов числа нет.
"""
import os
import subprocess
import sys
...
from storyboard35 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_{TAG}.mp4"
MUSIC = "/Users/vladimirkalajcidi/reels_good/audios/song1.mp3"
SR = 48000
...
def main():
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под синим выводом
        add(im, t, peak * 0.070)
    for t in CASCADES:                           # тики под каскадами элементов
        for k in range(3):
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_{TAG}.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=1.00dB,alimiter=limit=0.80:level=disabled",
