"""QA версий с другим хуком (ролик 36).
...
VD = "/Users/vladimirkalajcidi/reels_good/videos/36"
ORIG = SB.OUT
BODY_FRAMES = 1381 - HK.CUT_F
...
def body_identical(path, key, step=17):
    ...
    worst, checked = 0.0, 0
    for f in range(HK.CUT_F, 1381, step):
...
def seam_click(path, key):
    ...
    типичного скачка внутри дорожки. Склейка голоса — на кадре N-2 (J-cut)."""
    cfg = HK.HOOKS[key]
    wav = f"{HK.BUILD}/assets/_qa36seam.wav"
    ...
    os.remove(wav)
    rate = 1.02 if cfg.get("post_a") else 1.0
    i = int((cfg["frames"] - 2) / FPS / rate * 48000)      # склейка голоса
...
def pause_at_seam(key):
    ...
    Голос тела кладётся на кадр N-2, первое слово тела («у мужчины») звучит
    в теле на 4.110, то есть на (4.110 - CUT_A) позже начала вставленного
    куска. По огибающей атака чуть раньше, на 4.095 — пауза от этого только
    короче на 15мс и из диапазона не выходит.
    """
    cfg = HK.HOOKS[key]
    ...
    attack = (cfg["frames"] - 2) / FPS + (first - HK.CUT_A)
    rate = 1.02 if cfg.get("post_a") else 1.0
...
def deform(path, key):
    ...
    cfg = HK.HOOKS[key]
    t_orig = 38.60                     # план elder, все четыре ячейки на месте
    rate = 1.02 if cfg.get("post_a") else 1.0

    def grid_width(p, f):
        """Ширина квадрата исходов между ЦЕНТРАМИ левого и правого штрихов.

        По внешним краям мерить нельзя: bilinear-ресайз версии 2 размывает
        край, порог срезает его внутрь с обеих сторон и занижает масштаб
        (давало 1.0185 при 1.0208). Центр штриха от размытия не едет —
        размытие симметрично относительно него.
        """
        ...
        for k in range(9):
            ...
            h, w = img.shape[:2]
            y = int(round(982 * h / 1920))     # линия центров верхнего ряда
            strip = g[y - 2:y + 3, :].max(axis=0)
            idx = np.nonzero(strip > 120)[0]
            if not len(idx):
                continue
            runs = np.split(idx, np.where(np.diff(idx) > 1)[0] + 1)
            cen = lambda r: float((r * (strip[r] - 120.0)).sum()
                                  / (strip[r] - 120.0).sum())
            vals.append(cen(runs[-1]) - cen(runs[0]))
        c.release()
        return round(float(np.mean(vals)), 2) if vals else None

    f_orig = int(round(t_orig * FPS))
    f_ver = int(round((cfg["frames"] + (f_orig - HK.CUT_F)) / rate))
    a = grid_width(ORIG, f_orig)
    b = grid_width(path, f_ver)
    out = dict(grid_px_v1=a, grid_px_ver=b,
