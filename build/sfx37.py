"""Звук ролика 37: SFX + тихая музыка + исходная речь без резки.

Постель: whoosh на каждой из 27 склеек, импакт под ревилом красной верхней
стороны, под белым 1/2, под загоранием двух синих плиток и под синим итогом
2/3, тики под каскадами элементов сцены.
"""
import os
import subprocess
import sys
...
from storyboard37 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT, TAG
...
SR = 48000
GAIN_DB = "1.00dB"      # добор после loudnorm, подобран по замеру готового mp4
LIMIT = "0.80"
...
def main():
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под ревилами
        add(im, t, peak * 0.070)
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           f"volume={GAIN_DB},alimiter=limit={LIMIT}:level=disabled",
