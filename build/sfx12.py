"""Звук ролика 12: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard12 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
...
def main():
    tmp = f"{BUILD}/assets/_voice_pick.wav"
    ...
    for c in [s[0] for s in SHOTS[1:]]:          # whoosh на каждой склейке
        add(wh, c, peak * 0.040)
    for t in NUM_REVEALS:                        # импакт под числом-ревилом
        add(im, t, peak * 0.070)
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_pick.wav"
        ...
        xf = int(0.25 * SR)                      # петля с кроссфейдом
        ...
        track *= (vr / mr) * (10 ** (-19 / 20))  # -19 dB от RMS голоса
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_pick.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    # Однопроходный loudnorm на этом материале отдаёт -14.7 LUFS
                    # (delivery-specs §5). Догоняем усилением с лимитером:
                    # +0.7 dB -> -14.1 LUFS при истинном пике -1.7 dBFS. Замерено
                    # на готовом mp4, а не на миксе.
                    # level=disabled обязателен: у alimiter автоуровень включён по
                    # умолчанию и сам подтягивает сигнал к потолку — с ним те же
                    # +0.7 dB давали -12.0 LUFS и пик +0.2 dBFS, то есть клиппинг.
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=0.7dB,alimiter=limit=0.80:level=disabled",
