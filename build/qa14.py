"""QA ролика 14: зоны карточек, пересечение графики с субтитрами, наложение строк."""
import os
import sys
...
from storyboard14 import OUT, SHOTS, CAPS, DUR, GFX_ZONE, GFX_X, FACE_KINDS, shot_at
...
def line_overlaps():
    ...
    for i in range(len(CAPS) - 1):
        ...
        slot = render14.SLOTS[render14.slot_for(shot_at(CAPS[i][0])[2])]
...
def gfx_text_overlap():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render14.graphics_layer(kind, t - t0)
        if gl is None:
            continue
        cl = render14.caption_layer(t)
        if cl is None:
            continue
...
def gfx_in_zone():
    ...
    for f in range(0, int(DUR * FPS), 2):
        ...
        gl = render14.graphics_layer(kind, t - ts)
        if gl is None:
            continue
...
def caption_zone():
    """На планах с графикой субтитр не опускается ниже y600 (brand-kit §5)."""
    bad, sample = 0, []
    for f in range(0, int(DUR * FPS), 2):
        t = f / FPS
        kind = shot_at(t)[2]
        ...
        if cl is None:
            continue
        a = np.array(cl.split()[3])
        ys, _ = np.nonzero(a > ALPHA)
        if len(ys) and (ys.max() > 600 or ys.min() < 300):
            bad += 1
            sample.append((round(t, 2), kind, int(ys.min()), int(ys.max())))
    return bad, sample[:8]
...
if __name__ == "__main__":
    ...
    cz, czs = caption_zone()
    print(f"caption_out_of_zone_frames={cz} sample={czs}")
    tc, tcs = text_in_card()
