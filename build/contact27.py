"""Контакт-лист ключевых кадров ролика 27 — смысловая проверка графики глазами."""
import os
import sys
...
from storyboard27 import SRC, SHOTS, shot_at
from style import FPS
import render27

OUTDIR = sys.argv[1] if len(sys.argv) > 1 else "/private/tmp/claude-501/-Users-vladimirkalajcidi-reels-good/97ecda6d-1e88-4acd-b385-273a67d1faef/scratchpad/contact27"
...
for t in TIMES:
    ...
    def read_stock():
        c = cv2.VideoCapture(f"{render27.STOCK_DIR}/stock_{prm['clip']}.mp4")
        c.set(cv2.CAP_PROP_POS_MSEC, (prm.get("ss", 0) + (t - t0)) * 1000)
        _, s = c.read()
        c.release()
        return s

    canvas = render27.compose(kind, prm, fr, t, t0, read_stock)
