"""Звук ролика 26: SFX + тихая музыка + исходная речь без резки."""
import os
import subprocess
import sys
import wave
...
from storyboard26 import SHOTS, DUR, NUM_REVEALS, CASCADES, SRC, OUT
from sfx import low_whoosh, impact, tick, read_wav

BUILD = os.path.dirname(os.path.abspath(__file__))
VID = f"{BUILD}/assets/_video_tsp.mp4"
...
def main():
    tmp = f"{BUILD}/assets/_voice_tsp.wav"
    ...
    if os.path.exists(MUSIC):
        mw = f"{BUILD}/assets/_music_tsp.wav"
    ...
    if mx > 0.99:
        mix *= 0.99 / mx
    out_wav = f"{BUILD}/assets/_mix_tsp.wav"
    ...
    subprocess.run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                    ...
                    "-af", "highpass=f=65,loudnorm=I=-14:TP=-1.5:LRA=7,"
                           "volume=0.8dB,alimiter=limit=0.80:level=disabled",
